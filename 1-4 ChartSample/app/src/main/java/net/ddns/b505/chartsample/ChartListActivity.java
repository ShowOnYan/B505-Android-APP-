...
public class ChartListActivity extends AppCompatActivity {
    ...
    private ArrayList<ChartItem> list = new ArrayList<>(), list2 = new ArrayList<>(), list3 = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 建立單筆資料柱狀圖，並加入至圖表清單 p.s. 注意 單筆資料時，最後的參數『multple』需填false
        list2.add(new BarChartItem(chart.generateDataBar(dataset_id.get(0), dataset4, color), ChartListActivity.this, "發電量 (kWh)", getLabels(), yUnit, xUnit, false));
        // 建立多筆資料柱狀圖，並加入至圖表清單 p.s. 注意 多筆筆資料時，最後的參數『multple』需填true
        list2.add(new BarChartItem(chart.generateSixDataBar(dataset_id, totalList_bar, colors), ChartListActivity.this, "發電量 (kWh)", getLabels(), yUnit, xUnit, true));
        // 建立圓餅圖，並加入至圖表清單
        list3.add(new PieChartItem(chart.generateDataPie(labels , powerData), ChartListActivity.this, "用電數據", labels));
        ...
        ChartDataAdapter PieData = new ChartDataAdapter(ChartListActivity.this, list3);
        lv3.setAdapter(PieData);
